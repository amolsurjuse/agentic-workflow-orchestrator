...
#   3. Copies PR/project templates into <target>/.ai/
...
# Step 2: Copy agent files and shared rules.
...
# Step 4: Copy references and helper scripts.
...
for f in "$REPO_DIR/templates"/*; do
    ...
    if [ "$base" = "agent-commands.yml" ]; then
        continue
    fi
    cp "$f" "$TEMPLATES_DEST/"
    echo "      $base"
...
# Step 6: Copy agent command template.
...
# Step 8: Ensure generated paths are ignored.
...
echo "    3. Start: @01-launchpad <short ElectraHub requirement description>"
echo ""
echo "Done."
